/* conf.cmd: a configure script for autotools */
...
config: procedure
    ...
    /* Convert '\' to '/' */
    sTopSrcDir = translate( sTopSrcDir, '/', '\');

    /* Launch configure */
    'sh.exe' sTopSrcDir || '/configure' sArgs '2>&1 | tee conf.log'
    ec = rc
...
isbuilddir: procedure
    ...
    return stream( strip( sDir, 'T', '\') || '\config.status', 'c', 'query exists') \== ''
...
issrcdir: procedure
    ...
    return stream( strip( sDir, 'T', '\') || '\configure', 'c', 'query exists') \== ''
